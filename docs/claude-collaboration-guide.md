...
Claude Codeがこのテンプレートをユーザーの要求に合わせて変更する際の戦略：

#### 1. 段階的カスタマイズアプローチ

**Phase 1: 基本設定**
- プロジェクト名の変更（`project_name` → 実際の名前）
- パッケージ構造の調整
- 基本的な依存関係の追加

**Phase 2: ドメイン特化**
- プロジェクトタイプに応じた設定追加（Web/CLI/ML/モバイル）
- 特定フレームワークの統合（FastAPI等）
- ドメイン固有のベストプラクティス適用
...
Claude Codeは以下のパターンでCLAUDE.mdを動的に更新します：

```markdown
# 新しいルールの提案例
## AIDEV-SUGGESTION: [日付]
繰り返し指摘された以下の点をCLAUDE.mdに追加することを提案します：
- [具体的なルール]
- [理由]
- [適用範囲]
```

#### 3. CLAUDE.mdの情報密度の最適化戦略
